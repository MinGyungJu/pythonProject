...
'''
name = soup.select('#content .tabcont')
# print(name)
for na in name:
    # print(na.select('li strong')[0].text)
    구청이름.append(na.select('li strong')[0].text)
    # print(na.select('li')[0].text)   # 구청 주소
    구청주소.append(na.select('li')[0].text)
    # print(na.select('li')[1].text)   # 구청 전화번호
    구청전화번호.append(na.select('li')[1].text)
    # print(na.select('li')[2].text)   # 구청 홈페이지
    구청홈페이지.append(na.select('li')[2].text)

i =0

for i in range(0,len(구청이름)-1):
    print('구청이름 : ',구청이름[i])
    print('구청주소 : ',구청주소[i])
