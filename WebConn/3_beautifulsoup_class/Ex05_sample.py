...
soup = BeautifulSoup(rssUrl,'html.parser')
...
for i in soup:
    city = soup.select("location city")
    time = soup.select("location tmEf")
    wf = soup.select("location wf")

    for city, time, wf in zip(city,time,wf):
        print('도시 : ',city.text,'/ 시간 : ',time.text,' / 날씨 : ',wf.text)